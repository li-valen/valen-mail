...
/**
 * Re-exported so this route's own test file (`tests/opens-endpoint.test.ts`)
 * keeps importing `tokenMatches` from here unchanged. The implementation
 * moved to `src/compare.ts` in Plan 4 Task 1 so `api/tokens.ts` could share
 * it — see that module for the full reasoning.
 */
export { tokenMatches };
